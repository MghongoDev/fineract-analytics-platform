...
def build_parser() -> argparse.ArgumentParser:
    ...
    group.add_argument(
        "--all", action="store_true", help=f"Run every entity in order: {', '.join(DEFAULT_ORDER)}"
    )
    ingest.add_argument(
        "--parent-limit",
        type=int,
        default=None,
        help="Cap parent ids for child collections (loan_transactions); useful for smoke runs.",
    )
    ingest.add_argument(
        "--dry-run", action="store_true", help="Fetch, map and validate but roll back all writes."
    )
    ingest.add_argument(
        "--fail-fast", action="store_true", help="Stop at the first failing entity."
    )
...
def cmd_health(settings: Settings) -> int:
    ...
    print(
        json.dumps(
            {"component": "fineract_api", "healthy": api_ok, "base_url": settings.fineract.base_url}
        )
    )
    ok &= api_ok
    ...
    try:
        ...
        print(
            json.dumps(
                {"component": "postgres", "healthy": True, "replication_slots": slots}, default=str
            )
        )
