...
class FineractDataset:
    ...
    def __init__(
        self, clients: int = 400, loans: int = 700, seed: int = 42, as_of: date | None = None
    ):
    ...
    def _build_clients(self, count: int) -> list[dict]:
        ...
        for index in range(1, count + 1):
            ...
            rows.append(
                {
                    "id": index,
                    "accountNo": f"{index:09d}",
                    "externalId": f"EXT-C-{index:06d}",
                    "status": {
                        "id": 300 if active else 600,
                        "code": (
                            "clientStatusType.active" if active else "clientStatusType.closed"
                        ),
                    ...
                    "timeline": {
                        "submittedOnDate": _d(activation - timedelta(days=3)),
                        "activatedOnDate": _d(activation),
                    },
                    "officeId": office["id"],
                    "officeName": office["name"],
                    "staffId": officer["id"],
                    "staffName": officer["displayName"],
                    "legalForm": {
                        "id": 1 if index % 3 else 2,
    ...
    def _build_loans(self, count: int) -> list[dict]:
        ...
        for index in range(1, count + 1):
            ...
            status_id, status_code, status_value = self.random.choices(
                LOAN_STATUSES, weights=[4, 6, 45, 30, 5, 10]
            )[0]
            ...
            rows.append(
                {
                    ...
                    "loanOfficerName": officer["displayName"],
                    ...
                    "summary": {
                        ...
                        "interestOverdue": round(interest_outstanding * 0.35, 2)
                        if overdue_days
                        else 0,
                        ...
                        "totalOverdue": round(
                            (principal_outstanding + interest_outstanding) * 0.35, 2
                        )
                        if overdue_days
                        else 0,
    ...
    def _build_savings(self, count: int) -> list[dict]:
        ...
        for index in range(1, count + 1):
            ...
            rows.append(
                {
                    "id": index,
                    "accountNo": f"S{index:09d}",
                    "clientId": client["id"],
                    "clientName": client["displayName"],
                    "savingsProductId": product["id"],
                    "savingsProductName": product["name"],
                    ...
                    "timeline": {
                        "submittedOnDate": _d(opened - timedelta(days=2)),
    ...
    def _build_transactions(self) -> dict[int, list[dict]]:
        ...
        for loan in self.loans:
            ...
            per_interest = float(loan["summary"]["interestCharged"]) / max(
                1, loan["numberOfRepayments"]
            )
    ...
    def page(self, rows: list[dict], query: dict[str, list[str]]) -> Any:
        ...
        return {"totalFilteredRecords": len(rows), "pageItems": rows[offset : offset + limit]}
...
def serve(
    ...
    print(
        json.dumps(
            {
                "event": "mock_fineract_started",
                "port": port,
                "clients": len(Handler.dataset.clients),
                "loans": len(Handler.dataset.loans),
                "loan_transactions": total_tx,
                "base_url": f"http://localhost:{port}/fineract-provider/api/v1",
            }
        ),
        flush=True,
    )
